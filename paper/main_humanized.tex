...
Horizon-wise test error of comparators (principal supervised, feature retrieval and frozen TSFM and TRACE-RAF) is shown in Fig.~\ref{fig:mse_horizon} and Fig.~\ref{fig:mae_horizon}. All models show a late-horizon failure, but no model has an isolated failure that would be disqualifying for the aggregate comparison since the ranking changes with lead time. The DFEH score and fixed threshold (DFT) are plotted on the test timeline in Fig.~\ref{fig:drift_scores}. Figures~\ref{fig:forecast_case} and \ref{fig:forecast_event_case} show examples of ordinary and event-context figures, respectively, which are used to check rather than select models. All plots are regenerated from run 20260827T043457543402Z.
