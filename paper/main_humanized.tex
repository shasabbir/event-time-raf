...
\begin{abstract}
Meteorological variability, calendar effects, environmental events and distribution shift are the elements which complicate the short-horizon PM2.5 forecasting. While time-series foundation models offer transferable forecasting capabilities, the same time-series windows may not have the same local conditions that control urban air quality, as demonstrated by TimeRAF in the augmentation of a frozen forecaster. This study introduces Event-TimeRAF, an event-aware retrieval framework which consists of a Source-Audited Context Builder, a Leakage-Safe Event-Context Retriever and a Drift-Aware Forecast and Evidence Head. The framework includes: Official air-quality, weather, calendar, and storm-event records are aligned; retrieval is restricted to windows of training history that contain targets that precede the lookback contained in the query; feature-level XGBoost and forecast-level "frozen" Chronos-Bolt fusion are evaluated; a retrievable record of explanation evidence is stored. Evaluation is based on EPA AirData PM2.5 and NOAA Global Hourly weather data, and hash-checked NOAA Storm Events data from 2019 to 2024, with a 168 hour lookback and 24 hour horizon and for Los Angeles County. The best pre-specified supervised context baseline achieves a performance of MSE 26.185, MAE 3.125, RMSE 5.117, and $R^2$ 0.379, while the full feature-level model Event-TimeRAF achieves a performance of MSE 26.712. When frozen, Chronos-Bolt's MSE is 28.941 and when retrieved, it is 28.709, but the paired confidence interval does span zero. These values are re-calculated based on 7,199 test origins and prediction artifacts based on manifest. There is evidence for selective instead of uniform retrieval gains. Since Storm Events does not have publication time-stamps available, the event-conditioned results (findings) are not real-time forecasts but rather retrospective results of sensitivity.
\end{abstract}
...
The contributions of the main ones are listed below:
\begin{itemize}
...
There are varying techniques of forecasting, which have different rates of inductive bias, data required and computing time. autoregressive models and seasonal models are informative baseline models to consider, because failure modes are easily checked. Tree ensembles (e.g. XGBoost, LightGBM), build on this configuration, adding support for nonlinear lag, rolling, weather and calendar interactions, without needing to use a sequence encoder \cite{chen2016xgboost,ke2017lightgbm}. Recurrent LSTM and GRU models, however, learn stateful temporal representations \cite{hochreiter1997lstm,cho2014gru}. Reproducible comparison of time-series on heterogeneous domains of knowledge is also supported with large benchmark archive such as the Monash Forecasting Repository \cite{godahewa2021monash}. Along this line, transformer families propose longer dependencies, such as Informer to decrease the attention cost, Autoformer and FEDformer to decompose the time series, Crossformer and iTransformer to model the cross-variable structure, and TimesNet to rearrange the temporal variation as two-dimensional patterns \cite{zhou2021informer,wu2021autoformer,zhou2022fedformer,zhang2023crossformer,liu2024itransformer,wu2023timesnet}. These benefits don't necessarily mean that more complex architecture is always required. Simple decomposition or patch based inductive biases are still very competitive using DLinear and PatchTST \cite{zeng2023dlinear,nie2023patchtst}, while TimeMixer explicitly decomposes and mixes on the multiscale level \cite{wang2024timemixer}. Thus, Event-TimeRAF does not select a deep backbone as the default winner, but keeps seasonal and boosted tree baselines.
...
Retrieval augmentation is a dissociation between parametric knowledge and an external memory. The retrieve-then-condition pattern was pioneered by RAG and Dense Passage Retrieval in NLP \cite{lewis2020rag,karpukhin2020dpr}. In the context of time series, retrieval has been investigated using diffusion forecasting, and assisted generation workflows \cite{liu2024ratd,ang2024tsgassist}. TimeRAF is the most similar method: they build a time-series knowledge base, use an MLP dual-encoder retriever, find the top k candidates and incorporate the candidate embeddings into a frozen TSFM via Channel Prompting \cite{zhang2025timeraf}. It ablations retrieval quality, integration, number of candidates, composition of the knowledge-base and selection of backbone.

Event-TimeRAF is based on the same idea as external-memory, but is designed to answer a different experimental question. It leverages transparent random, cosine and event-context similarity instead of the learned dual encoder, and it leverages feature- or forecast-level fusion instead of internal Channel Prompting. The difference means that the current implementation can't be called a copy of TimeRAF: it's an audited environmental adaptation, which is meant to determine if contextual historical analogues assist a local PM2.5 task.
...
\begin{equation}
    ...
    \label{eq:lookback}
\end{equation}
Here, L is the length of the lookback. Suppose that $W_t$ are weather covariates that are aligned with the lookback window, $C_t$ are calendars features, and E\_t are event context variables that are available at the forecast origin under the recorded source assumption. The target in Equation~\eqref{eq:target} is the target for the input to Equation~\eqref{eq:lookback}.
\begin{equation}
...
The common output layer of two verified forecasting paths and the evidence output is the DFEH. A direct path is defined as the concatenation of the context, retrieval, drift and future-calendar features for 24 independently-trained XGBoost regressors, as illustrated in Figure~\ref{fig:explanation_flow}. The second track is a value of the retrieved forecast, mixed with the forecast of the frozen Chronos-Bolt at a validation-selected weight. The current code also includes a drill of a validation-only drift-state selector which is not represented in this manuscript in the context of a manifest-backed run.
...
\end{equation}
$u_{t,h}\in\mathbb{R}^{151}$ for the complete M09 model and each $f_h$ is an XGBoost regressor trained with squared error objective. The design allows for each forecast horizon to have a supervised function, but shares a common origin representation. The verified 151 features (85 context, 51 retrieval, six drift and nine future-calendar) are also included in equation~\eqref{eq:direct_heads}.

Fusion stage with a light weight drift-conditioned selector, tabular. It determines the strongest validation candidates for drift and non-drift validation origins, before using the selected source for each of the test origins, as indicated in Equation~\eqref{eq:drift_gate}.
\begin{equation}
    \hat{Y}^{G}_t =
    \begin{cases}
    \hat{Y}^{a}_{t}, & D_t = 0,\\
    ...
    \label{eq:drift_gate}
\end{equation}
where the a and the b have been selected only from among validation errors. This gate is not training a new model, but is to see if it is possible to use a different forecaster in each regime based on a transparent drift-specific selection rule. The current notebook procedure is given in Equation~\eqref{eq:drift_gate}. It will not be used in the comparison until it has been added to the manifest, predictions, metrics and checksums are added to the manifest and it is rerun completely.
...
\end{equation}
Equations~\eqref{eq:drift_raw}--\eqref{eq:drift_score} are equivalent to the detector which was used: the reference statistics are set using training data, the threshold 0.90 is set using validation data and test data are transformed without refitting. The binary $D_t$ is the state which is applied to Equation~\eqref{eq:drift_gate}.
...
\end{equation}
The mean absolute validation residual $\bar e_{\mathrm{val}}$ and the mean horizon-wise spread of the retrieved trajectories $\bar\sigma_{R,t}$ are used here. So that doesn't produce a calibrated prediction interval, it produces a diagnostic scale. For each explanation, it is created from the stored fields, not from an unrestricted language model and each event statement is not causal, but rather contextual. Only as an evidence-field definition, Equation~\eqref{eq:uncertainty_proxy} is not used for constructing confidence intervals.
...
The following section is a description of the experiment reported. For models M00 to M11, the selection of models were fixed using the training and validation partitions before looking at the results of the test. Selector used for a later post-hoc is not included in the reported comparisons as it was added post-manifest. Run identifier 20260723T112033170131Z was used to test for artifact consistency.
...
NOAA Storm Events annual detail archives \cite{noaastormevents2026} were extracted and loaded as environmental events from the data, taking care to ensure that the events were not duplicated. The experiment was performed using a locally staged cache with NOAA annual archives with no changes and a source manifest file that listed the official NOAA urls, the byte count, and the SHA-256 hash value for each archive. No delivery mode change was required during the audit and the delivery mode \texttt{verified\_official\_noaa\_cache} was retained. A total of 117 event days, 2,192 source-overlap days and three qualifying event categories were identified from the event audit. Since the NOAA Storm Events doesn't provide publication timestamps that can be exposed in machine-readable format, event-aware outputs make a retrospective assumption of when an event was available based on the event start time. Are not in-and-of-themselves real-time event forecasts. The resulting readiness status is given in Table~\ref{tab:data_audit}. Figure~\ref{fig:context_builder} shows the source audit window and window contract.
...
EPA AirData, NOAA Global Hourly/ISD and NOAA Storm Events provide the raw public records and download pages \cite{epaairdata2026,noaaisd2026,noaastormevents2026}. The local execution environment did not have access to direct NCEI access so a locally staged NOAA Storm Events cache was used instead and all cached NOAA files were compared to the source manifest file generated. The source code can be found here: \url{https://github}.com/shasabbir/event-time-raf. The implementation consists of the following packages: Python 3.12.13, NumPy, pandas, scikit-learn, XGBoost, PyTorch and chronos-forecasting package and the following artifacts are contained in the repository: configuration, notebooks, source modules and tests for reproducing the pipeline when the public source files are available.
...
The results on the test-set are given in Table~\ref{tab:main_results}. The best supervised result from the pre-specified ones is M04 which can be achieved from the PM2.5 history, weather and calendar features with MSE 26.185, MAE 3.125, RMSE 5.117, and $R^2$ 0.379. In this run, the full feature-level Event-TimeRAF model (M09) achieves MSE 26.712 and MAE 3.149, and event-context retrieval and drift features do not improve the all-origin error over M04 in this run. A selector is not included because the result tables and prediction file of the selector do not match with the run manifest.
...
The summary of the explanation layer is given in Table~\ref{tab:explainability_outputs}. This design is audit friendly since each explanation statement is mapped to one of the model features, one of the retrieved time-series windows, an official event record or a drift component. Not causal: Event evidence is reported as evidence and the drift flag as uncertainty warning.
...
The general interpretation is that the conventional supervised context features are very strong for this task of Los Angeles County 24-hour PM2.5. Retrieval can be used as an independent historical-pattern baseline, and yields a small positive MSE change when combined with frozen Chronos-Bolt. The current feature-level Event-TimeRAF design, however, is not the best overall design, compared to the best XGBoost context baseline. It should thus not be seen as a superior forecasting model, but as an implemented and audited event-aware retrieval framework with selective gains, not a uniformly superior forecasting model. The following is not part of this reading of the manifest: post-manifest artifacts.
