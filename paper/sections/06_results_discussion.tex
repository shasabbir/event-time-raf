...
two Chronos variants. All subsets pass the minimum-size gate. The event subset
has lower MSE than the non-event subset for these models, which should not be
interpreted as events being easier in general; it reflects the particular 2024
test-period event hours retained by the audit. On drift-flagged origins, the
full Event-TimeRAF model is worse than the context baseline, while
retrieval-augmented Chronos is slightly better than frozen Chronos by MSE. This
again indicates that the present event/drift feature fusion is not sufficient to
dominate the context baseline.
...
Code availability & Source code is not publicly available. \\
\bottomrule
\end{tabular}
\end{table}
