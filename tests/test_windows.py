...
def test_window_attrition_reconciles_accepted_origins(modeling_frame, cfg):
    dataset = build_window_dataset(modeling_frame, cfg)
    attrition = window_attrition_table(modeling_frame, dataset, cfg)
    accepted = attrition.loc[attrition["stage"] == "accepted", "count"].item()
    assert accepted == len(dataset.metadata)
    possible = attrition.loc[attrition["stage"] == "possible_forecast_origins", "count"].item()
    exclusion_stages = {
        "excluded_split_boundary",
        "missing_lookback",
        "missing_target",
        "missing_origin_features",
        "missing_future_calendar",
        "accepted",
    }
    assert attrition.loc[attrition["stage"].isin(exclusion_stages), "count"].sum() == possible
